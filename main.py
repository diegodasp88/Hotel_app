...
def main(page: ft.Page):
    ...
    def clear_search_reserv(e):
        ...
        # colocar cursor no campo id
        id_reserv_search_ref.current.focus()
    ...
    def submit_reserv(e):
        ...
        if client_reserv_ref and rooms_reserv_ref and checkin_ref and checkout_ref and status_reserv_ref:
            ...
            render_rooms() # atualizar dropdown de quartos na reserva
            id_reserv_submit_ref.current.update()
            reserv_table_ref.current.update()
    ...
    def update_reserv(e):
        ...
        upd_reserv = Reservation(
                reserv_id=int(id_reserv_search_ref.current.value),
                reserv_owner=name_reserv_search_ref.current.value,
                reserv_room=int(room_reserv_search_ref.current.value),
                check_in=checkin_reserv_search_ref.current.value,
                check_out=checkout_reserv_search_ref.current.value,
                reserv_status=status_reserv_search_ref.current.value
            )
        if id_reserv_search_ref.current.value:
            id = id_reserv_search_ref.current.value
    ...
    def remove_reserv_data_by_id(e):
        """Remove a reserva no banco de dados pelo ID."""
        if id_reserv_search_ref.current.value:
            id = id_reserv_search_ref.current.value
            page.close(confirm_reserv_removal)
            db.remove_reserv_by_id(id=id)
            page.open(success_removal)
            render_reserv_table() # atualizar tabela de reservas
            clear_search_reserv(e) # limpar formulário de busca de reservas
            db.update_room_status() # atualizar status do quarto após reserva
            ...
            reserv_table_ref.current.update()
            render_clients_dropdown() # atualizar o dropdown de clientes na reserva
            id_reserv_submit_ref.current.update() # atualizar o campo ID no cadastro de reservas
            id_reserv_search_ref.current.focus() # colocar o cursor no campo ID
        else:
            page.open(err_id)

    def get_reserv_data_by_id(e):
        """Coleta os dados da reserva pelo ID do banco de dados e preenche o formulário."""
        id = id_reserv_search_ref.current.value
        try:
            if id_reserv_search_ref.current.value:
                reserv = db.get_reserv_by_id(id)
                # Preencher os campos
                name_reserv_search_ref.current.value = reserv["Titular"]
                room_reserv_search_ref.current.value = reserv["Quarto"]
                checkin_reserv_search_ref.current.value = reserv["Check-in"]
                checkout_reserv_search_ref.current.value = reserv["Check-out"]
                status_reserv_search_ref.current.value = reserv["Status"]
                # Ativar os campos e botões
                name_reserv_search_ref.current.disabled = False
                room_reserv_search_ref.current.disabled = False
                checkin_reserv_search_ref.current.disabled = False
                checkout_reserv_search_ref.current.disabled = False
                status_reserv_search_ref.current.disabled = False
                # Atualizar os campos
                name_reserv_search_ref.current.update()
                room_reserv_search_ref.current.update()
                checkin_reserv_search_ref.current.update()
    ...
    limpar_busca_reserva = filled_buttons.clear_button(on_click=clear_search_reserv)
    atualizar_reserva = filled_buttons.update_button(on_click=lambda e: page.open(confirm_reserv_update))
    remover_reserva = filled_buttons.remove_button(on_click=lambda e: page.open(confirm_reserv_removal))
    
    # Variáveis dos AlertDialogs
    err_id = alert_dialogs.alert_err(
        content=ft.Text("ID inexistente."),
        actions=[text_buttons.ok_button(on_click=lambda e: page.close(err_id))])

    err_search = alert_dialogs.alert_err(
        content=ft.Text("Preencha o campo ID."),
    ...
    confirm_update = alert_dialogs.alert_confirm(
        content=ft.Text("Você realmente deseja atualizar os dados desse cliente?"),
        actions=[
    ...
    confirm_reserv_update = alert_dialogs.alert_confirm(
        content=ft.Text("Você realmente deseja atualizar os dados dessa reserva?"),
        actions=[
            text_buttons.yes_button(on_click=update_reserv),
            text_buttons.no_button(on_click=lambda e: page.close(confirm_reserv_update))
    ])

    confirm_reserv_removal = alert_dialogs.alert_confirm(
    ...
    id_reserv_search = textfields.id_reserv_search(ref=id_reserv_search_ref)
                                                ...
                                                content=ft.Container(
                                                    ...
                                                    content=ft.Column(
                                                        controls=[
                                                            ...
                                                            ft.Row([id_reserv_search, # campo ID da reserva
                                                                    ft.IconButton( # Botão Busca
                                                                        bgcolor="#083b21",
                                                                        icon=ft.Icons.SEARCH,
                                                                        tooltip="Buscar",
                                                                        style=ft.ButtonStyle(bgcolor={ft.ControlState.HOVERED: "#24965b", ft.ControlState.DEFAULT: "#083b21"}),
                                                                        on_click=get_reserv_data_by_id
                                                                    )
                                                            ]),
                                                            ft.Row([name_reserv_search]), # campo nome do titular da reserva
                                                            ft.Row([room_reserv_search]), # campo quarto da reserva
                                                            ft.Row([checkin_reserv_search, ft.Container(width=30), checkout_reserv_search]), # campos check-in e check-out
                                                            ft.Row([status_reserv_search, ft.Container(width=30), limpar_busca_reserva, atualizar_reserva, remover_reserva]) # campo status e botões
                                                        ]
                                                    )
                                                )
                                            )
