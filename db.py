...
def remove_reserv_by_id(id: int) -> dict:
...
def update_room_status():
    ...
    if not reserve_table.all(): # Nenhuma reserva: todos os quartos ficam disponíveis
        rooms_table.update({"Status do quarto": rm.RoomStatus.AVAILABLE})
    for reserva in reserve_table.all():
        ...
        # Atualiza todos os outros quartos como disponíveis
        for room in rooms_table.all():
            num = room.get("Quarto")
            if num not in quartos_reservados:
                rooms_table.update({"Status do quarto": rm.RoomStatus.AVAILABLE}, Query().Quarto == num)
